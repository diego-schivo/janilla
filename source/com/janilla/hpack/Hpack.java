...
//		System.out.println("headers=" + d.headers);
		...
		System.out.println(s);
		d = new Decoder();
		d.decodeHeaderList(parseHex(s).iterator());
		System.out.println("dynamicTable=" + d.dynamicTable);
		System.out.println("headers=" + d.headers);
	...
	static class Decoder {
		...
		void decodeHeaderList(PrimitiveIterator.OfInt bytes) {
			while (bytes.hasNext())
				decodeHeader(bytes);
		}
	...
	static class Encoder {
		...
		IntStream encodeHeader(Header header) {
			...
			if (hh == null)
				hh = dynamicTable.headers(header.name());
			IndexAndHeader h;
			if (hh != null) {
	...
	static class Table {
		...
		void add(Header header) {
			...
			map.compute(header.name(), (k, v) -> {
				if (v == null)
					v = new ArrayList<>();
				v.add(new IndexAndHeader(list.size(), header));
				return v;
			});
		}
	}
