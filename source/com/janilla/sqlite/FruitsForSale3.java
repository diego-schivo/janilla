	...
	public static void main(String[] args) {
		...
		try (var ch = new TransactionalByteChannel(
			...
			d.perform(() -> {
				...
				IO.println(Arrays.toString(i.select("Orange", "CA").mapToLong(x -> (long) x[2]).peek(IO::println)
						.mapToObj(x -> t.select(x).findFirst().get()).mapToDouble(x -> (double) x[3]).toArray()));

				IO.println(Arrays.toString(i.select("Peach").mapToLong(x -> (long) x[2]).peek(IO::println)
						.mapToObj(x -> t.select(x).findFirst().get()).mapToDouble(x -> (double) x[3]).toArray()));
