	...
	public static void main(String[] args) {
		...
		try (var ch = new TransactionalByteChannel(
			...
			d.perform(() -> {
				...
				var p = t.rows().peek(x -> IO.println(Arrays.toString(x))).filter(x -> x[1].equals("Peach"))
						.mapToDouble(x -> (double) x[3]).findFirst().getAsDouble();
				IO.println("p=" + p);

				p = t.select(4l).mapToDouble(x -> (double) x[3]).findFirst().getAsDouble();
				IO.println("p=" + p);
