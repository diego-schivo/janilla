...
public record Record2(int[] header, byte[][] body) {
...
//		int[] tt;
//		{
//			var l = Varint.get(b[0]);
//			var ii = IntStream.builder();
//			while (b[0].position() < l)
...
//	}
	...
	public static Stream<Value> fromBytes2(Stream<ByteBuffer> buffers) {
		var bi = buffers.iterator();
		var b = new ByteBuffer[] { bi.next() };
		int[] tt;
		{
...
//		IO.println("tt=" + Arrays.toString(tt));
		return Arrays.stream(tt).mapToObj(t -> new Value(t, switch (t) {
		case 0 -> null;
		case 1, 2, 3, 4, 5, 6 -> {
...
//								IO.println("b[0]=" + b[0].remaining());
	...
	public record Value(int type, byte[] content) {
	}
}
