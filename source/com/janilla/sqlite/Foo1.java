		...
		try (var ch = new TransactionalByteChannel(
			...
			for (var x : ss) {
				...
				d.perform(() -> {
					switch (x.charAt(0)) {
					case 'I':
						d.indexBTree("t1", "table").insert(new Object[] { a, b }, new Object[0]);
						break;
					case 'D':
						d.indexBTree("t1", "table").delete(a);
						break;
					default:
			...
			d.perform(() -> {
				try {
					Files.write(Path.of("ex1b.txt"), d.indexBTree("t1", "table").rows().map(Arrays::toString).toList());
				} catch (IOException e) {
					throw new UncheckedIOException(e);
				}
				return null;
