	...
	public static void main(String[] args) {
		...
		try (var ch = new TransactionalByteChannel(
			...
			d.perform(() -> {
				...
				var i = d.createIndex("Idx1", "fruitsforsale", "fruit");

				t.insert(_ -> new Object[] { 1l, "Orange", "FL", 0.85 });
				i.insert(new Object[] { "Orange", 1l });
				t.insert(_ -> new Object[] { 2l, "Apple", "NC", 0.45 });
				i.insert(new Object[] { "Apple", 2l });
				t.insert(_ -> new Object[] { 4l, "Peach", "SC", 0.60 });
				i.insert(new Object[] { "Peach", 4l });
				t.insert(_ -> new Object[] { 5l, "Grape", "CA", 0.80 });
				i.insert(new Object[] { "Grape", 5l });
				t.insert(_ -> new Object[] { 18l, "Lemon", "FL", 1.25 });
				i.insert(new Object[] { "Lemon", 18l });
				t.insert(_ -> new Object[] { 19l, "Strawberry", "NC", 2.45 });
				i.insert(new Object[] { "Strawberry", 19l });
				t.insert(_ -> new Object[] { 23l, "Orange", "CA", 1.05 });
				i.insert(new Object[] { "Orange", 23l });

				{
					var id = i.select("Peach").mapToLong(x -> (long) x[1]).findFirst().getAsLong();
					IO.println("id=" + id);
					var p = t.select(id).mapToDouble(x -> (double) x[3]).findFirst().getAsDouble();
					IO.println("p=" + p);
				}

				i.select("Orange").mapToLong(x -> (long) x[1]).forEach(id -> {
					IO.println("id=" + id);
					var p = t.select(id).mapToDouble(x -> (double) x[3]).findFirst().getAsDouble();
					IO.println("p=" + p);
				});
				...
				IO.println(Arrays.toString(
						i.select("Orange").mapToLong(x -> (long) x[1]).mapToObj(x -> t.select(x).findFirst().get())
								.filter(x -> x[2].equals("CA")).mapToDouble(x -> (double) x[3]).toArray()));
