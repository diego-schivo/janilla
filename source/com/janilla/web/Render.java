...
public @interface Render {
	...
	public static abstract class Renderer implements BiFunction<Object, HttpExchange, String> {
		...
		protected static Pattern textCommentAttribute = Pattern.compile(">\\$\\{([\\w.-]*)\\}</" + "|"
				+ "<!--\\$\\{([\\w.-]*)\\}-->" + "|" + "[\\w-]+=\"\\$\\{([\\w.-]*)\\}\"");

		protected static String interpolate(String template, Object context) {
			return textCommentAttribute.matcher(template).replaceAll(x -> {
				...
				var r = v != null ? v.getClass().getAnnotation(Render.class) : null;
				if (r != null)
					try {
						v = r.value().getConstructor().newInstance().apply(v, null);
