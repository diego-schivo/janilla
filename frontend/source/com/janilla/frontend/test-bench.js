...
export default class TestBench extends WebComponent {
    ...
    connectedCallback() {
        ...
        this.customState.tests = Object.entries(tests).map((x, i) => ({
            value: i,
            text: x[0]
        }));
    ...
    handleLoad = event => {
        ...
        const s = this.customState.tests[k];
    ...
    handleSubmit = event => {
        ...
        this.customState.tests.forEach(x => x.class = null);
        this.requestDisplay();
    }
    ...
    async updateDisplay() {
        ...
        this.appendChild(this.interpolateDom({
            $template: "",
            testItems: this.customState.tests.map(x => ({
                $template: "test-item",
                ...x
            }))
        }));
